...
/*
This is for statemtents like
x*10
x+10
where it doesnot return anything
so it is basically a wrapper around expression
*/
...
// for representing infix expression like a+b*c
...
// for handling boolean operations
type Boolean struct {
	Token token.Token
	Value bool
}
...
func (fi *FunctionLiteral) expressionNode()      {}
func (fi *FunctionLiteral) TokenLiteral() string { return fi.Token.Literal }
func (fi *FunctionLiteral) String() string {
	var out bytes.Buffer
	params := []string{}
...
// for accessing data in array
type IndexExpression struct {
	Token token.Token //the [ token
	Left  Expression  //the identifier being accessed
	Index Expression  //value for accessing
}
